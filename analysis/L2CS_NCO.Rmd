...
results <- read_csv(paste0("../.output/L2CS_NCO/", "/results.csv")) %>%
  mutate(video = str_replace(video, "_processed\\.mp4", ""))

study_data <- read_csv(paste0("../.output/L2CS_NCO/", "number_comparison_output.csv")) %>%
  mutate(
    video = str_replace(vid_fname, "\\.webm", ""),
...
results <- results %>% left_join(study_data, by = c("video"))
...
result <- results_all_faces %>% filter(face_count == 1)
